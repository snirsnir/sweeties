    ...
    return new Promise(resolve => {
        ...
        img.onload = () => {
            ...
            if (formData.celebrantAge) {
                ...
                const ageW  = ctx.measureText(formData.celebrantAge).width + S * 0.3;
                const ageH  = ageSize * 1.5;
                const ageR  = ageSize * 0.18;
                const ageX  = cx - ageW / 2;
                const ageT  = ageY - ageH / 2;
                ctx.save();
                ctx.globalAlpha = 0.55;
                ctx.fillStyle   = '#fff';
                ctx.beginPath();
                ctx.moveTo(ageX + ageR, ageT);
                ctx.lineTo(ageX + ageW - ageR, ageT);
                ctx.arcTo(ageX + ageW, ageT, ageX + ageW, ageT + ageR, ageR);
                ctx.lineTo(ageX + ageW, ageT + ageH - ageR);
                ctx.arcTo(ageX + ageW, ageT + ageH, ageX + ageW - ageR, ageT + ageH, ageR);
                ctx.lineTo(ageX + ageR, ageT + ageH);
                ctx.arcTo(ageX, ageT + ageH, ageX, ageT + ageH - ageR, ageR);
                ctx.lineTo(ageX, ageT + ageR);
                ctx.arcTo(ageX, ageT, ageX + ageR, ageT, ageR);
                ctx.closePath();
                ctx.fill();
                ctx.restore();
                ctx.fillStyle = PINK;
            ...
            ctx.fillText(footerText, cx, H * 0.855, W * 0.90);
